...
from research.vin_peisuke.graph import *

from research.vin_peisuke.algorithm.obstacle import *
...
def main():
    ...
    max_obs_size = 1.0
    num_trajs = 7
    max_samples = num_domains * num_trajs * max_traj_len // 2

    im_data = np.zeros((max_samples, size_2, size_1), dtype=np.uint8)
    value_data = np.zeros((max_samples, size_2, size_1), dtype=np.uint8)
    state_xy_data = np.zeros((max_samples, 2), dtype=np.int32)
    label_data = np.zeros((max_samples,), dtype=np.int32)
    ...
    while dom <= num_domains:
        ...
        for i in range(len(states_xy)):
            if len(states_xy[i]) > 0:
                ...
                #    cv2.imshow("test", cv2.resize(255 - test_img * 255, (300, 300),
                # interpolation=cv2.INTER_NEAREST))
                #    key = cv2.waitKey(0)
                #    if key == ord('q'):
                #        return
        print("%d / %d" % (dom, num_domains))
        dom += 1

    data = {
        'im': im_data[0:num_samples],
        'value': value_data[0:num_samples],
        'state': state_xy_data[0:num_samples],
        'label': label_data[0:num_samples]
    }
